...
static void do_mode_0(unsigned short blocks, const char *buf, size_t len)
{
	...
	data = (char *)(buf + 4);
	AnimFrame *frame = malloc(sizeof(AnimFrame));

	for (i = 0; i < blocks; i++) {
		char fname[100];

		sprintf(fname, "PIC%03lu.GIF", i);
		frame->x0 = 0;
		frame->y0 = 0;
		frame->width = x;
		frame->height = y;
		...
		frame->pixels = data;
		frame->comment = '\0';
		dump_gif(fname, x, y, 1, frame, colors, 0, pal);
		data += x * y;
	}
	free(frame);
}
			 ...
			 unsigned char mode)
{
	...
	for (i = 0; i < blocks; i++) {
		...
		frame->pixels = data;
		frame->comment = '\0';
		dump_gif(fname, x, y, 1, frame, colors, first_color, pal);
