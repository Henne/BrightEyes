...
int dump_tga(ImageSet* img, char* prefix) {
	...
	for (j = first_color;  j < colors;  j++) {
	    tga_palette[j * 3 + 0] = img->globalPalette[j - first_color].r << 2;
	    tga_palette[j * 3 + 1] = img->globalPalette[j - first_color].g << 2;
	    tga_palette[j * 3 + 2] = img->globalPalette[j - first_color].b << 2;
	}
