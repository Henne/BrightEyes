				...
				if (item2 != 0) {
					...
					if ((item2 == item1) && item_stackable(get_itemsdat(item1))) {
						...
						if (!can_hero_use_item(hero, item2)) {
							...
							sprintf((char*)g_dtp2, get_ttx(221), (char*)hero + HERO_NAME2,
							...
							if (!can_item_at_pos(item2, pos1)) {
								if (is_in_word_array(item2, g_items_pluralwords))
									...
									sprintf((char*)g_dtp2, get_ttx(222),
								...
								if (two_hand_collision(hero, item2, pos1)) {
									...
									sprintf((char*)g_dtp2, get_ttx(829), (char*)hero + HERO_NAME2);
									GUI_output((char*)g_dtp2);
...
void print_item_description(Bit8u *hero, signed short pos)
{
	...
	if (host_readw(inventory_p + INVENTORY_ITEM_ID) == ITEM_KUKRIS_DAGGER || host_readw(inventory_p + INVENTORY_ITEM_ID) == ITEM_KUKRIS_MENGBILAR ||
		...
		strcat((char*)g_dtp2, get_ttx(548));
	}
...
void pass_item(Bit8u *hero1, signed short old_pos1, Bit8u *hero2, signed short pos2)
{
	...
	if (pos2 < 7) {
		if (!can_hero_use_item(hero2, item1)) {
			...
			sprintf((char*)g_dtp2, get_tx2(67), (char*)(hero2 + HERO_NAME2));
			GUI_output((char*)g_dtp2);
			return;
		}
	}
...
void equip_belt_ani(void)
{
	...
	do_border((Bit8u*)ds_readd(VGA_MEMSTART), 209, 79, 215, 89, 9);

	do_fill_rect((Bit8u*)ds_readd(VGA_MEMSTART), 209, 79, 215, 89, 0);

	wait_for_vsync();
	wait_for_vsync();
	wait_for_vsync();

	do_fill_rect((Bit8u*)ds_readd(VGA_MEMSTART), 189, 69, 235, 99, 0);

	do_border((Bit8u*)ds_readd(VGA_MEMSTART), 189, 69, 235, 99, 9);

	wait_for_vsync();
	wait_for_vsync();
	wait_for_vsync();

	do_fill_rect((Bit8u*)ds_readd(VGA_MEMSTART), 169, 59, 255, 109, 0);

	do_border((Bit8u*)ds_readd(VGA_MEMSTART), 169, 59, 255, 109, 9);

	wait_for_vsync();
	wait_for_vsync();
	wait_for_vsync();

	do_fill_rect((Bit8u*)ds_readd(VGA_MEMSTART), 164, 54, 260, 114, 0);

	do_border((Bit8u*)ds_readd(VGA_MEMSTART), 164, 54, 260, 114, 9);

	wait_for_vsync();
	wait_for_vsync();
	wait_for_vsync();

	do_fill_rect((Bit8u*)ds_readd(VGA_MEMSTART), 159, 49, 263, 117, 0);

	do_border((Bit8u*)ds_readd(VGA_MEMSTART), 159, 49, 263, 117, 9);
	...
	g_current_ani = ds_writebs(DNG_AREA_LOADED, ds_writebs(CITY_AREA_LOADED, -1));
}
