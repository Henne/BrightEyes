...
void spell_exposami(void)
{
	...
	if (count) {
		/* Intro text */
		strcpy((char*)ds_readd(DTP2), get_tx(31));
		...
		for (i = 0; count - 1 > i; i++) {
			...
			if (count - 2 > i) {
				strcat((char*)ds_readd(DTP2), get_tx(28));		/* "," */
			}
		}

		if (count > 1) {
			strcat((char*)ds_readd(DTP2), get_tx(29));		/* "AND" */
		}
		...
		strcat((char*)ds_readd(DTP2), get_tx(30));			/* "." */
	} else {
		/* no more hidden enemies */
		strcpy((char*)ds_readd(DTP2), get_tx(32));
	}
}
...
void spell_odem_arcanum(void)
{
	...
	if (id) {
		...
		if (inventory_magic(get_spelluser() + pos * SIZEOF_INVENTORY + HERO_INVENTORY + INVENTORY_ITEM_ID)) {
			...
			sprintf((char*)ds_readd(DTP2), get_tx(81),
			...
			sprintf((char*)ds_readd(DTP2), get_tx(82),
				(char*)GUI_names_grammar((signed short)0x8000, id, 0));
		}
	}
}
...
void spell_duplicatus(void)
{
	...
	/* prepare the message */
	sprintf((char*)ds_readd(DTP2), get_tx(84), (char*)get_spelluser() + HERO_NAME2,
		(char*)GUI_get_ptr(host_readbs(get_spelluser() + HERO_SEX), 0));
...
void spell_blitz(void)
{
	...
	if (host_readbs(get_spelluser() + HERO_ENEMY_ID) < 10) {
		...
		if (get_spelltarget() == get_spelluser()) {
			...
			/* prepare the message */
			sprintf((char*)ds_readd(DTP2), get_tx(86), (char*)get_spelltarget() + HERO_NAME2);
		}
	} else {
		/* cast an enemy */
		...
		/* prepare the message */
		sprintf((char*)ds_readd(DTP2), get_tx(85),
...
void spell_ecliptifactus(void)
{
	...
	if (rounds != -1) {
		...
		if (host_readws(get_spelluser() + HERO_AE) >= ae) {
			...
			/* prepare the message */
			sprintf((char*)ds_readd(DTP2), get_tx(88), (char*)(get_spelluser() + HERO_NAME2),
...
void spell_eisenrost(void)
{
	...
	if (host_readbs(get_spelluser() + HERO_ENEMY_ID) < 10) {
			...
			if (!id) {
				...
				if (inventory_broken(get_spelltarget() + HERO_INVENTORY + HERO_INVENTORY_SLOT_RIGHT_HAND * SIZEOF_INVENTORY)) {
					...
					if (host_readbs(get_spelltarget() + (HERO_INVENTORY + HERO_INVENTORY_SLOT_RIGHT_HAND * SIZEOF_INVENTORY + INVENTORY_BF)) > 0) {
						...
						sprintf((char*)ds_readd(DTP2), get_tx(92),
							(char*)GUI_names_grammar((signed short)0x8000, id, 0),
		...
		{
			sprintf((char*)ds_readd(DTP2), get_tx(89));
		} else {
			/* check if weapon is already broken */
			if (host_readbs(get_spelltarget_e() + ENEMY_SHEET_BROKEN) != 0) {
				...
				/* prepare message */
				sprintf((char*)ds_readd(DTP2), get_tx(91),
					(char*)GUI_names_grammar((signed short)0x8000, host_readbs(get_spelltarget_e()), 1));
			}
		}
	}
...
void spell_ignifaxius(void)
{
	...
	if ((ds_readws(AUTOFIGHT) == 0) && (host_readbs(get_spelluser() + HERO_NPC_ID) == 0)) {
		...
		sprintf((char*)ds_readd(DTP2), get_tx(93), host_readbs(get_spelluser() + HERO_LEVEL) + 1);
...
void spell_plumbumbarum(void)
{
	...
	if (host_readbs(get_spelluser() + HERO_ENEMY_ID) < 10) {
		...
		if (get_spelltarget() == get_spelluser()) {
			...
			/* prepare the message */
			sprintf((char*)ds_readd(DTP2), get_tx(94), (char*)get_spelltarget() + HERO_NAME2);
			}
	...
	/* prepare the message */
	sprintf((char*)ds_readd(DTP2), get_tx(95),
...
void spell_saft_kraft(void)
{
	...
	/* prepare message */
	sprintf((char*)ds_readd(DTP2), get_tx(96), (char*)get_spelltarget() + HERO_NAME2);
...
void spell_scharfes_auge(void)
{
	...
	sprintf((char*)ds_readd(DTP2), get_tx(97), (char*)get_spelltarget() + HERO_NAME2);
}
