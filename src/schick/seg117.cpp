...
void hunt_viper(void)
{
	...
	if (l_di) {
		...
		/* print a message */
		sprintf((char*)g_dtp2, get_tx2(26), hero_i + HERO_NAME2);
		...
		for (i = 0; i <= 6; i++, hero += SIZEOF_HERO) {
				...
				!overboard[i])
			{
...
#endif
				{
					...
					sprintf((char*)g_dtp2, get_tx2(30), (char*)hero + HERO_NAME2);
					GUI_output((char*)g_dtp2);
				}
...
void hunt_bison(void)
{
	...
	if (answer == 1) {
		...
		sprintf((char*)g_dtp2, get_tx2(37), (char*)hero + HERO_NAME2);
		GUI_output((char*)g_dtp2);
...
void hunt_rhino(void)
{
	...
	if (answer == 1) {
		...
		hero = get_hero(get_random_hero());
		sprintf((char*)g_dtp2, get_tx2(43), (char*)hero + HERO_NAME2);
		GUI_output((char*)g_dtp2);
		sub_hero_le(hero, dice_roll(2, 6, 0));
		add_hero_ap(hero, 6);
	}
