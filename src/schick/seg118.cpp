...
void tevent_078(void)
{
	...
	if (!get_current_season())
	{
		...
		if (answer == 1)
		{
			...
			do {
				/* ORIENTATION + 8 */
				if (test_skill((Bit8u*)get_first_hero_available_in_group(), TA_ORIENTIERUNG, 8) > 0)
				{
...
void tevent_051(void)
{
	...
	if (test_skill((Bit8u*)get_first_hero_available_in_group(), TA_FAEHRTENSUCHEN, 4) > 0 && !ds_readb(TEVENT051_FLAG))
	{
		...
		if (!TRV_follow_trail_question())
		{
			...
			do {
				answer = GUI_dialogbox((Bit8u*)ds_readd(DTP2), NULL, get_tx2(67), 2,
							get_tx2(68),
							get_tx2(69));
			} while (answer == -1);
		...
		do {
			answer = GUI_dialogbox((Bit8u*)ds_readd(DTP2), NULL, get_tx2(70), 2,
						get_tx2(71),
						get_tx2(72));
		} while (answer == -1);
...
void tevent_052(void)
{
	...
	do {
		...
		do {
			answer = GUI_dialogbox((Bit8u*)ds_readd(DTP2), NULL, get_tx2(73), 2,
						get_tx2(74),
						get_tx2(75));
		} while (answer == -1);
...
void tevent_125(void)
{
	...
	do {
		answer = GUI_dialogbox((Bit8u*)ds_readd(DTP2), NULL, get_tx2(57), 3,
					get_tx2(58),
					get_tx2(59),
					get_tx2(60));
	} while (answer == -1);
