...
void INF_jurge_hjore(signed short informer, signed short state)
{
	...
	if (!informer) {
		...
		if (!state) {
			ds_writew(DIALOG_NEXT_STATE, (gs_informer_flags[INFORMER_JURGE] == 1 || ds_readb(TLK_TAV_FOLLOWINFORMER) != 0) ? 44 : 43);
		} else if (state == 11) {
			ds_writew(DIALOG_NEXT_STATE, has_intro_letter() ? 36 : 37);
		} else if (state == 14) {
			ds_writew(DIALOG_NEXT_STATE, has_intro_letter() ? 39 : 40);
		} else if (state == 19) {
			ds_writew(DIALOG_NEXT_STATE, ds_readb(TLK_TAV_FOLLOWINFORMER) != 0 ? 1 : (!ds_readb(TLK_TAV_FOLLOWINFORMER) && gs_informer_flags[INFORMER_JURGE] != 0 ? 2 : 3));
		} else if (state == 20) {
		...
		if (!state) {
			ds_writew(DIALOG_NEXT_STATE, !gs_informer_flags[INFORMER_HJORE] ? 16 : (gs_informer_flags[INFORMER_HJORE] == 2 ? 1 : 5));
		} else if (state == 5) {
...
void INF_yasma_umbrik_isleif(signed short informer, signed short state)
{
	...
	if (!informer) {
		...
		if (!state) {
			ds_writew(DIALOG_NEXT_STATE, gs_informer_flags[INFORMER_YASMA] == 2 ? 1 : 2);
		} else if (state == 15) {
		...
		if (!state) {
			ds_writew(DIALOG_NEXT_STATE, gs_informer_flags[INFORMER_UMBRIK] == 2 ? 1 : 2);
		} else if (state == 2) {
			ds_writew(DIALOG_NEXT_STATE, ds_readb(UMBRIK_QUEST_GORAH) != 0 ? 15 : (!gs_informer_flags[INFORMER_UMBRIK] ? 24 : 3));
		...
		if (!state) {
			ds_writew(DIALOG_NEXT_STATE, gs_informer_flags[INFORMER_ISLEIF] == 2 ? 1 : 2);
		} else if (state == 2) {
...
void INF_ragna_beorn_algrid(signed short informer, signed short state)
{
	...
	if (informer == 0) {
		...
		if (!state) {
			ds_writew(DIALOG_NEXT_STATE, gs_informer_flags[INFORMER_RAGNA] == 2 ? 1 : 3);
		} else if (state == 4) {
			...
			if (!state) {
				ds_writew(DIALOG_NEXT_STATE, gs_informer_flags[INFORMER_BEORN] == 2 ? 1 : 2);
			} else if (state == 2) {
		...
		if (!state) {
			ds_writew(DIALOG_NEXT_STATE, ds_readb(TLK_TAV_FOLLOWINFORMER) ? 23 : (gs_informer_flags[INFORMER_ALGRID] == 2 ? 1 : 22));
		} else if (state == 2) {
			/* mark ALGRID TRONDESDOTTER as done */
			gs_informer_flags[INFORMER_ALGRID] = 2;

			ds_writew(DIALOG_NEXT_STATE, gs_informer_flags[INFORMER_JURGE] == 2 ? 3 : 4);
		} else if (state == 3) {
			ds_writew(DIALOG_NEXT_STATE, gs_algrid_treborns_hint ? 5 : 6);
		} else if (state == 4) {
			ds_writew(DIALOG_NEXT_STATE, gs_algrid_treborns_hint ? 7 : 8);
		} else if (state == 14) {
			/* make TIOMAR SWAFNILDSSON known */
			if (!gs_informer_flags[INFORMER_TIOMAR]) gs_informer_flags[INFORMER_TIOMAR] = 1;
...
void INF_eliane_tiomar(signed short informer, signed short state)
{
	if (!informer) {
		...
		if (!state) {
			...
			ds_writew(DIALOG_NEXT_STATE, ds_readb(QUEST_NAMELESS_DONE) && gs_informer_flags[INFORMER_ELIANE] != 2 ? 2 : 3);
		...
		if (!state) {
			...
			ds_writew(DIALOG_NEXT_STATE, gs_informer_flags[INFORMER_UMBRIK] == 2 ? 46 : 47);
		}
...
void INF_olvir_asgrimm(signed short informer, signed short state)
{
	...
	if (!informer) {
		...
		if (!state) {
			ds_writew(DIALOG_NEXT_STATE, gs_informer_flags[INFORMER_OLVIR] == 2 ? 1 : 2);
		} else if (state == 2) {
...
void INF_treborn_unicorn(signed short informer, signed short state)
{
	...
	if (!informer) {
		...
		if (!state) {
			ds_writew(DIALOG_NEXT_STATE, gs_informer_flags[INFORMER_TREBORN] != 0 ? -1 : 1);
		} else if (state == 1) {
		...
		if (!state) {
			ds_writew(DIALOG_NEXT_STATE, gs_informer_flags[INFORMER_UNICORN] == 2 ? 1 : 2);
		} else if (state == 2) {
...
void INF_swafnild_unicorn(signed short informer, signed short state)
{
	if (!informer) {
		...
		if (state == 1) {
			ds_writew(DIALOG_NEXT_STATE, gs_informer_flags[INFORMER_SWAFNILD] == 2 ? 3 : 7);
		} else if (state == 4) {
