	...
	do {
		...
		if (done == 0)
		{
			...
			if (!ds_readb(TEVENT114_OLIMONE_FLAG))
			{
				...
				GUI_dialogbox((char*)ds_readd(DTP2), get_tx(52), get_tx2(25), 0);
				GUI_dialogbox((char*)ds_readd(DTP2), get_tx(52), get_tx2(26), 0);
	...
	if (!ds_readb(TEVENT122_FLAG))
	{
		...
		if (answer == 1)
		{
			...
			if (!TRV_fight_event(FIGHTS_F122, 122))
			{
				...
				GUI_dialogbox((char*)ds_readd(DTP2), get_tx(68), get_tx2(37), 0);
				GUI_dialogbox((char*)ds_readd(DTP2), get_tx(68), get_tx2(63), 0);
			}
		} else {
			/* decline to help */
			sub_hero_ap_all(20);
