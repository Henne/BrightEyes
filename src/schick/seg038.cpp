	...
	for (i = 0; i < 4; i++) {
		...
		while (dist >= 0) {
			...
			while (success == 0) {
				...
				{
					cb_or_dist_entry = *(dist_table_ptr + 25 * backtrack_y + backtrack_x);
...
//should be static
signed int FIG_count_direction_changes_of_path(const int8_t *path_ptr)
{
				...
				if (*(dist_table_ptr + (y * 25) + x) == (dist - 1)) {
					...
					for (i = 0; i < 4; i++) {
						...
						if ((new_y < 24) && (new_y >= 0) && (new_x < 24) && (new_x >= 0)) {
							...
							cb_or_dist_entry = *(dist_table_ptr + new_y * 25 + new_x);
