...
void PHX_drogentempel(void)
{
	...
	if ((ds_readds(DAY_TIMER) >= HOURS(15)) && (ds_readds(DAY_TIMER) <= HOURS(19))) {
		...
		if (ds_readb(DRUG_TIMER) && (nr_heroes * 2000 <= money)) {
			...
			GUI_dialogbox((char*)ds_readd(DTP2), get_tx2(15), get_tx2(14), 0);
		...
		if (ds_readb(GREMOB_INVITED) != 0) {
			...
			if (ds_readb(DRUG_TIMER) && (nr_heroes * 2500 <= money)) {
				...
				GUI_dialogbox((char*)ds_readd(DTP2), get_tx2(16), get_tx2(28), 0);
				...
				if (answer == 1) {
					...
					GUI_dialogbox((char*)ds_readd(DTP2), get_tx2(16), get_tx2(26), 0);
					...
					if (answer == 1) {
						GUI_dialogbox((char*)ds_readd(DTP2), get_tx2(16), get_tx2(27), 0);
						ds_writeb(GREMOB_INVITED, 1);
					} else {
						GUI_dialogbox((char*)ds_readd(DTP2), get_tx2(16), get_tx2(8), 0);
						ds_writeb(DRUG_TIMER, 10);
					}
				}
			}
