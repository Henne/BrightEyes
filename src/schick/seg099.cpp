...
void spell_beherrschung(void)
{
	...
	if (!hero_renegade(get_spelltarget())) {
		...
		if (get_spelltarget() == get_spelluser()) {
			strcpy((char*)ds_readd(DTP2), get_tx(0));
...
void spell_band(void)
{
	if (host_readbs(get_spelluser() + HERO_ENEMY_ID) >= 10) {
		...
		if (get_spelltarget() == get_spelluser()) {
			...
			/* prepare message */
			sprintf((char*)ds_readd(DTP2), get_tx(6), (char*)get_spelltarget() + HERO_NAME2);
		}
	}
}
...
void spell_somnigravis(void)
{
	...
	if (get_spelltarget() == get_spelluser()) {
		...
		/* prepare message */
		sprintf((char*)ds_readd(DTP2), get_tx(13), (char*)get_spelltarget() + HERO_NAME2);
	}
}
...
void spell_balsam(void)
{
		...
		((host_readbs(get_spelluser() + HERO_NPC_ID) != 0) && ds_readws(IN_FIGHT) != 0))
	{
		...
		/* ask question */
		le_to_heal = GUI_input((char*)ds_readd(DTP2), 2);
...
void spell_hexenspeichel(void)
{
	...
	le = GUI_input((char*)ds_readd(DTP2), 2);
...
void spell_klarum_purum(void)
{
	...
	if (!poison) {
		...
		/* prepare message */
		sprintf((char*)ds_readd(DTP2), get_tx(21), (char*)get_spelltarget() + HERO_NAME2);
		ds_writew(SPELL_SPECIAL_AECOST, 0);
		return;
	}
...
void spell_ruhe_koerper(void)
{
	...
	/* prepare message */
	sprintf((char*)ds_readd(DTP2), get_tx(51), (char*)get_spelltarget() + HERO_NAME2);
}
...
void spell_tiere_heilen(void)
{
	...
	ae = GUI_input((char*)ds_readd(DTP2), 2);
...
char* spell_analues(void)
{
	...
	if (ds_readws(IN_ACADEMY) != 99) {
		...
		/* prepare message */
		sprintf((char*)ds_readd(DTP2), get_tx(53), (char*)get_spelluser() + HERO_NAME2, (char*)ds_readd(TEXT_OUTPUT_BUF));
	}
