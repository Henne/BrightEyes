...
void FIG_do_enemy_action(struct enemy_sheet* p_enemy, const signed int enemy_id)
{
	...
	if (g_in_fight) {
		...
		if (p_enemy->target_object_id < 10) {
			...
			if ((is_in_byte_array(target_enemy->actor_sprite_id, g_double_size_actor_sprite_id_table)) && (l17 == 0))
			{
				...
				*g_dtp2 = '\0';
				...
				if (p_enemy->target_object_id) {
					...
					if (l13 > 0) {
						...
						if ((p_enemy->actor_sprite_id != ACTOR_SPRITE_ID_HEXE__FEMALE) && (p_enemy->actor_sprite_id != ACTOR_SPRITE_ID_HEXE__MALE) && (p_enemy->target_object_id > 0)) {
