...
void tevent_074(void)
{
	...
	if (!gs_tevent074_flag) {
		...
		if (answer == 1) {
			...
			/* fight */
			gs_tevent074_fight_flag = TRV_fight_event(FIGHT_ID_F074, 74);
	...
	if (!gs_tevent075_flag) {
		...
		if (answer == 1) {
			...
			if (answer == 1) {
				...
				ret = TRV_fight_event(FIGHT_ID_F075_A, 75);
	...
	if (!gs_tevent077_flag) {
		...
		if (answer == 1) {
			...
			if (test_talent(get_first_hero_available_in_group(), TA_KRIEGSKUNST, 6) <= 0) {
				...
				g_fig_initiative = 1;
				gs_tevent077_fight_flag = TRV_fight_event(FIGHT_ID_F077, 77);
